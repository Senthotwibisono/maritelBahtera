...
<div id="layoutProfile">
    @include('master.master_layout.detil.profile')
</div>
    ...
    <div class="card">
        <div class="card-content">
            ...
            <div class="card-body" id ="contentLayout">
                @include('master.master_layout.detil.content')
            </div>
        </div>
    </div>
</section>
...
@include('master.master_layout.detil.js')
...
<script>
  ...
  async function getDataItem(data) {
    ...
    async function updateProfileLayout(button){
        const result = await confirmation();
        if (result.isConfirmed) {
            buttonLoading(button);
            const id = document.getElementById('layoutId').value;
            const name = document.getElementById('layoutName').value;
            const remark = document.getElementById('layoutRemark').value;
            const data = {
                id,
                name,
                remark
            };
            const url = '{{route('master.layout.profileUpdate')}}';
            const response = await globalResponse(data,url);
            hideButton(button);
            if (response.ok) {
                const hasil = await response.json();
                if (hasil.success) {
                    await successHasil(hasil).then(() => {
                        const id = {{$layout->id}};
                        const url = `{{ route('master.layout.profileDetil', ['id' => '__id__']) }}`.replace('__id__', id);
                        $("#layoutProfile").load(url);
                        initMainDrag();
                        initDetilDrag();
                    });
                    return;
